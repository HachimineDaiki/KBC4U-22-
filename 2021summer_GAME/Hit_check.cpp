...
void Ground_model_hit_check(VECTOR MoveVector) {
	DrawFormatString(100, 260, GetColor(255, 0, 0), "引数MoveVector  x %.1f  y %.1f  z %.1f", MoveVector.x, MoveVector.y, MoveVector.z);
	// 移動前の座標を保存
	st_model_hit.OldPos = sph[0].pos;
	DrawFormatString(100, 280, GetColor(255, 0, 0), "Old  x %.1f  y %.1f  z %.1f", st_model_hit.OldPos.x, st_model_hit.OldPos.y, st_model_hit.OldPos.z);

	P_move();

	// 移動後の座標を算出
	st_model_hit.NowPos = VAdd(sph[0].pos, MoveVector);
	DrawFormatString(100, 300, GetColor(255, 0, 0), "NowPos  x %.1f  y %.1f  z %.1f", st_model_hit.NowPos.x, st_model_hit.NowPos.y, st_model_hit.NowPos.z);
	/*DrawFormatString(100, 250, GetColor(255, 255, 255), "NowPos %.1f , %.1f . %.1f", st_model_hit.NowPos.x, st_model_hit.NowPos.y, st_model_hit.NowPos.z);*/
	...
	//DrawFormatString(100, 330, GetColor(255, 0, 0), "判定  x %.1d", st_model_hit.HitDimNum);
