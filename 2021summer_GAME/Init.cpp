...
void Sph_init() {
	...
	for (int i = 0; i < 2; i++) {
		...
		sph[0].front = 200;//プレイヤーの前方200
		sph[0].right = 200;//プレイヤーの右200
		sph[0].left = -200;//プレイヤーの左200
	}
...
void PlanetInit() {
	...
	planet[3].radius = 69635;//太陽
	//planet[4].radius = 127420;//地球の半径
	...
	planet[0].pos = VGet(0, 36597 - (planet[0].radius * 2), earth_pos_z + 38000);//月
	planet[1].pos = VGet(0, 192466 - (planet[1].radius * 2), earth_pos_z + 400000);//金星
	planet[2].pos = VGet(0, 339155 - (planet[2].radius * 2), earth_pos_z + 770000);//水星
	planet[3].pos = VGet(0, 525050, 1451646);//太陽

	//planet[3].pos = VGet(0, 622587 /*- (planet[3].radius * 2)*/, earth_pos_z + 1500000);//太陽
	//planet[4].pos = VGet(0, -10000, 50000);//地球
